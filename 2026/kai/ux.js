...
export default class Ux {
  ...
  static building3(params = {}) {
    ...
    if (params.controls) {
      div._controls = {};
      for (const key in params.controls) {
        const control = params.controls[key];
        if (control.onclick) {
          div._controls[key] = Ux.button({ text: control.text, parent: div, onclick: control.onclick, disabled: control.disabled });
        } else {
          const span = document.createElement('span');
          div.appendChild(span);
          div._controls[key] = span;
        }
      }
    }
    ...
    div.redraw = (params = {}) => {
      for (const key in params.controls) {
        const control = params.controls[key];
        const element = div._controls[key];
        if (element) {
